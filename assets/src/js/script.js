...
const body = doc.body
const ka = "Traffic_Forecast"

function getPagesViewed() {
  return (doc.cookie.match(/_Traffic_Forecast_pages_viewed=([^;]+)/) ?? [""]).pop().split('a')
}
...
function init () {
  // window.Traffic_Forecast might be missing if the active theme is not calling wp_head()
  if (!win[ka]) return
