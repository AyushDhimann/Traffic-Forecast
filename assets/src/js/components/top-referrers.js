...
export default class TopReferrers extends Component {
  ...
  render () {
    ...
    return (
      <div className='box Traffic-fade top-referrers'>
        <div className='box-grid head'>
          <div className=''>
            <span className='muted'>#</span>
            {__('Referrers', 'Traffic-Forecast')}

            <Pagination offset={offset} limit={limit} total={items.length} onUpdate={this.loadData} />
          </div>
          <div className='amount-col'>{__('Visitors', 'Traffic-Forecast')}</div>
          <div className='amount-col'>{__('Pageviews', 'Traffic-Forecast')}</div>
        </div>
        <div className='body'>
          {items.map((p, i) => (
