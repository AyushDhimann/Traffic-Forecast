...
const startOfWeek = parseInt(window.Traffic_Forecast.start_of_week, 10)
      ...
      <div className='date-nav'>
        ...
        <div className='date-picker-ui' style={{ display: open ? '' : 'none' }}>
          <div className='date-quicknav cf'>
            <span onClick={this.onQuickNavClick('prev')} className='prev dashicons dashicons-arrow-left' title={__('Previous', 'Traffic-Forecast')} />
            <span className='date'>
              <span>{format(startDate, 'MMM d, yyyy')}</span>
              <span> &mdash; </span>
              <span>{format(endDate, 'MMM d, yyyy')}</span>
            </span>
            <span onClick={this.onQuickNavClick('next')} className='next dashicons dashicons-arrow-right' title={__('Next', 'Traffic-Forecast')} />
          </div>
          <div className='flex'>
            <div className='date-presets'>
              <div>
                <label for='ka-date-presets'>{__('Date range', 'Traffic-Forecast')}</label>
              ...
              <div>
                <label>{__('Custom', 'Traffic-Forecast')}</label>
