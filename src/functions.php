...
function maybe_collect_request() {
	...
	if ( ! isset( $_GET['action'] ) || $_GET['action'] !== 'Traffic_Forecast_collect' || ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
		return;
	}
...
function collect_request() {
	...
	if ( isset( $_GET['sc'] ) && (int) $_GET['sc'] === 1 ) {
		...
		setcookie( '_Traffic_Forecast_pages_viewed', $cookie, time() + 6 * HOUR_IN_SECONDS, '/' );
	}
...
function get_buffer_filename() {
	if ( defined( 'Traffic_Forecast_BUFFER_FILE' ) ) {
		return Traffic_Forecast_BUFFER_FILE;
	}
...
function get_settings() {
	...
	$settings = (array) get_option( 'Traffic_Forecast_settings', array() );
...
function admin_bar_menu( $wp_admin_bar ) {
	...
	$wp_admin_bar->add_node(
		array(
			'parent' => 'site-name',
			'id' => 'Traffic-Forecast',
			'title' => esc_html__( 'Forecast', 'Traffic-Forecast' ),
			'href' => admin_url( '/index.php?page=Traffic-Forecast' ),
		)
	);
}
...
function widgets_init() {
	...
	register_widget( 'TrafficForecast\Widget_Most_Viewed_Posts' );
}
...
function using_custom_endpoint() {
	if ( defined( 'Traffic_Forecast_CUSTOM_ENDPOINT' ) ) {
		return Traffic_Forecast_CUSTOM_ENDPOINT;
	}

	return get_option( 'Traffic_Forecast_use_custom_endpoint', false );
}
