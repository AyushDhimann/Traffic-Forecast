...
class Aggregator {
	...
	public function init() {
		add_action( 'Traffic_Forecast_aggregate_stats', array( $this, 'aggregate' ) );
	...
	public function add_interval( $intervals ) {
		$intervals['Traffic_Forecast_stats_aggregate_interval'] = array(
			'interval' => 1 * 60, // 1 minute
			'display'  => esc_html__( 'Every minute', 'Traffic-Forecast' ),
		);
		return $intervals;
	}

	public function setup_scheduled_event() {
		if ( ! wp_next_scheduled( 'Traffic_Forecast_aggregate_stats' ) ) {
			wp_schedule_event( time() + 60, 'Traffic_Forecast_stats_aggregate_interval', 'Traffic_Forecast_aggregate_stats' );
		}
	}
	...
	public function aggregate() {
		...
		// insert site stats
		$sql = $wpdb->prepare( "INSERT INTO {$wpdb->prefix}Traffic_Forecast_site_stats(date, visitors, pageviews) VALUES(%s, %d, %d) ON DUPLICATE KEY UPDATE visitors = visitors + VALUES(visitors), pageviews = pageviews + VALUES(pageviews)", array( $date, $site_stats['visitors'], $site_stats['pageviews'] ) );
		$wpdb->query( $sql );
		...
		if ( count( $post_stats ) > 0 ) {
			...
			$sql          = $wpdb->prepare( "INSERT INTO {$wpdb->prefix}Traffic_Forecast_post_stats(date, id, visitors, pageviews) VALUES {$placeholders} ON DUPLICATE KEY UPDATE visitors = visitors + VALUES(visitors), pageviews = pageviews + VALUES(pageviews)", $values );
			$wpdb->query( $sql );
		}
		...
		if ( count( $referrer_stats ) > 0 ) {
			...
			$sql           = $wpdb->prepare( "SELECT id, url FROM {$wpdb->prefix}Traffic_Forecast_referrer_urls r WHERE r.url IN({$placeholders})", $referrer_urls );
			...
			if ( count( $new_referrer_urls ) > 0 ) {
				...
				$sql          = $wpdb->prepare( "INSERT INTO {$wpdb->prefix}Traffic_Forecast_referrer_urls(url) VALUES {$placeholders}", $values );
				$wpdb->query( $sql );
			...
			$sql          = $wpdb->prepare( "INSERT INTO {$wpdb->prefix}Traffic_Forecast_referrer_stats(date, id, visitors, pageviews) VALUES {$placeholders} ON DUPLICATE KEY UPDATE visitors = visitors + VALUES(visitors), pageviews = pageviews + VALUES(pageviews)", $values );
			$wpdb->query( $sql );
		}
	...
	private function update_realtime_pageview_count( $pageviews ) {
		$counts = (array) get_option( 'Traffic_Forecast_realtime_pageview_count', array() );
		...
		update_option( 'Traffic_Forecast_realtime_pageview_count', $counts, false );
	}
	...
	private function ignore_referrer_url( $url ) {
		...
		if ( $blocklist === null ) {
			$blocklist = file( Traffic_Forecast_PLUGIN_DIR . '/data/referrer-blocklist', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES );

			// add result of filter hook to blocklist so user can provide custom domains to block through simple array
			$custom_blocklist = apply_filters( 'Traffic_Forecast_referrer_blocklist', array() );
		...
		return apply_filters( 'Traffic_Forecast_ignore_referrer_url', false, $url );
	}
	...
	public function normalize_url( $url ) {
		...
		$aggregations = apply_filters( 'Traffic_Forecast_url_aggregations', $aggregations );
