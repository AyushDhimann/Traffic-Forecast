...
class Rest {
	...
	function register_routes() {
		register_rest_route(
			...
			array(
				...
				'permission_callback' => function () {
					return current_user_can( 'view_Traffic_Forecast' );
				},
			)
		);
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => function () {
					return current_user_can( 'manage_Traffic_Forecast' );
				},
			)
		);
	...
	public function get_stats( \WP_REST_Request $request ) {
		...
		$sql        = $wpdb->prepare( "SELECT date, visitors, pageviews FROM {$wpdb->prefix}Traffic_Forecast_site_stats s WHERE s.date >= %s AND s.date <= %s", array( $start_date, $end_date ) );
	...
	public function get_posts( \WP_REST_Request $request ) {
		...
		$sql        = $wpdb->prepare( "SELECT s.id, SUM(visitors) AS visitors, SUM(pageviews) AS pageviews, p.post_title FROM {$wpdb->prefix}Traffic_Forecast_post_stats s LEFT JOIN {$wpdb->posts} p ON p.ID = s.id WHERE s.date >= %s AND s.date <= %s GROUP BY s.id ORDER BY pageviews DESC, s.id ASC LIMIT %d, %d", array( $start_date, $end_date, $offset, $limit ) );
	...
	public function update_settings( \WP_REST_Request $request ) {
		...
		update_option( 'Traffic_Forecast_settings', $new_settings, true );
		return true;
	}
	...
	public function reset_data( \WP_REST_Request $request ) {
		global $wpdb;
		$wpdb->query( "TRUNCATE {$wpdb->prefix}Traffic_Forecast_site_stats;" );
		$wpdb->query( "TRUNCATE {$wpdb->prefix}Traffic_Forecast_post_stats;" );
		$wpdb->query( "TRUNCATE {$wpdb->prefix}Traffic_Forecast_referrer_stats;" );
		$wpdb->query( "TRUNCATE {$wpdb->prefix}Traffic_Forecast_referrer_urls;" );
