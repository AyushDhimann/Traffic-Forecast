...
class Script_Loader {
	...
	public function init() {
		...
		add_action( 'amp_print_Forecast', array( $this, 'print_amp_Forecast_tag' ) );
	}
	...
	public function maybe_enqueue_script( $echo = false ) {
		 ...
		 * @param bool
		 */
		$load_script = apply_filters( 'Traffic_Forecast_load_tracking_script', true );
		...
		if ( false === $echo ) {
			...
			wp_enqueue_script( 'Traffic-Forecast', plugins_url( 'assets/dist/js/script.js', Traffic_Forecast_PLUGIN_FILE ), array(), Traffic_Forecast_VERSION, true );
		} else {
			$this->print_js_object();
			echo '<script defer src="', plugins_url( sprintf( 'assets/dist/js/script.js?ver=%s', Traffic_Forecast_VERSION ), Traffic_Forecast_PLUGIN_FILE ), '"></script>';
		}
	...
	private function get_tracker_url() {
		if ( defined( 'Traffic_Forecast_CUSTOM_ENDPOINT' ) && Traffic_Forecast_CUSTOM_ENDPOINT ) {
			return site_url( Traffic_Forecast_CUSTOM_ENDPOINT );
		}
		...
		return using_custom_endpoint() ? site_url( '/Traffic-Forecast-collect.php' ) : admin_url( 'admin-ajax.php?action=Traffic_Forecast_collect' );
	}
	...
	public function print_js_object() {
		...
		$script_config         = array(
			...
			'dnt' => apply_filters( 'Traffic_Forecast_honor_dnt', true ),
		);
		echo '<script>window.Traffic_Forecast = ', json_encode( $script_config ), ';</script>';
	}

	public function print_amp_Forecast_tag() {
		$settings = get_settings();
		$post_id = $this->get_post_id();
		$tracker_url = $this->get_tracker_url();
		$posts_viewed = isset( $_COOKIE['_Traffic_Forecast_pages_viewed'] ) ? explode( ',', $_COOKIE['_Traffic_Forecast_pages_viewed'] ) : array();
		$data = array(
		...
		echo sprintf( '<amp-Forecast><script type="application/json">%s</script></amp-Forecast>', json_encode( $config ) );
	}

	public function add_async_attribute( $tag, $handle ) {
		if ( $handle !== 'Traffic-Forecast' || stripos( $tag, 'defer' ) !== false ) {
			return $tag;
		}
