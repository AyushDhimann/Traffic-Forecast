=== Traffic Forecast ===
Contributors: Trailblazers, DvanKooten
Donate link: https://opencollective.com/Traffic-Forecast
Tags: Forecast, statistics, stats, Traffic
...
Privacy-friendly Forecast for your WordPress site

== Description ==

Traffic Forecast is a privacy-friendly Forecast plugin for WordPress. It does not use any external services, so data about your visitors is never shared with any third-party company.
...
Stop sharing visitor data with third-party companies making money off that same data. Stop unnecessarily slowing down your website. Traffic Forecast lets you focus on what is important and gives you all the essential metrics, while respecting the privacy of your visitors.
...
- Help fund the plugin through our [OpenCollective intiative](https://opencollective.com/Traffic-Forecast).

== Installation ==

1. In your WordPress admin area, go to **Plugins > New Plugin**, search for **Traffic Forecast** and click **Install now**.
1. Alternatively, [download the plugin files](https://downloads.wordpress.org/plugin/Traffic-Forecast.trunk.zip) and upload the contents of `Traffic-Forecast.zip` to your plugins directory, which usually is `/wp-content/plugins/`.
1. Activate the plugin. Traffic Forecast will start recording stats right away.
1. Access your Forecast by browsing to *Dashboard > Forecast* in your WordPress admin area.
...
No, the data never leaves your website. That's (part of) what makes Traffic Forecast such a great choice if you value true privacy.
...
In fact, because the plugin does not depend on any external services it is usually much faster than third-party Forecast tools.
...
First configure Traffic Forecast to not use any cookies by default.

Then configure your cookie consent plugin to add the following code for visitors that have given consent:

`
<script>
if (window.Traffic_Forecast) window.Traffic_Forecast.use_cookie = true;
</script>
`

### How do I give users access to the dashboard page?
You can use a plugin like [User Role Editor](https://wordpress.org/plugins/user-role-editor/) to grant the `view_Traffic_Forecast` and `manage_Traffic_Forecast` capabilities to any user rolou can delete the plugin again after adding the capability.
...
### I only see an empty page when viewing the Forecast dashboard.
Traffic Forecast was recently added to EasyPrivacy, a popular filter list which is used in many ad-blockers. It blocks both the tracking script and the dashboard script, but we are working with EasyPrivacy team to get this resolved.

Until then, please ensure to whitelist your own domain in your ad-blocker settings.

### How can I help fund Traffic Forecast?
Please visit the [Traffic Forecast project page on OpenCollective](https://opencollective.com/Traffic-Forecast).


== Screenshots ==

1. Traffic Forecast' dashboard to view your website statistics.
...
- You can now call `Traffic_Forecast.trackPageview( postId )` to manually track a pageview. This is especially useful for Single Page Applications.
...
- Add filter `Traffic_Forecast_url_aggregations` to modify or add URL aggregations for referrer URL's.
- Created an initiative on OpenCollective for others to help fund development and support costs of Traffic Forecast. Please visit https://opencollective.com/Traffic-Forecast for more details.
...
- Delete option `Traffic_Forecast_use_custom_endpoint` on plugin uninstall.
- Only load dashboard-widget.js if user has required capability for viewing stats.
- Add constant `Traffic_Forecast_CUSTOM_ENDPOINT` for  [disabling the custom endpoint](https://github.com/Trailblazers/Traffic-Forecast/blob/master/code-snippets/disable-custom-endpoint.php) entirely. This filter can also be used to [manually install the endpoint file to a different file location](https://github.com/Trailblazers/Traffic-Forecast/blob/master/code-snippets/use-different-custom-endpoint.php).
...
- Add class attribute to cron warning so it can be hidden using CSS by targeting `.Traffic-Forecast-cron-warning`.
...
- Add WP CLI command for aggregating stats without having to go through WP Cron: `wp Traffic-Forecast aggregate` 
...
- Add function for manually printing tracking script: `<?php Traffic_analyics_tracking_script(); ?>`
...
If your WordPress root directory is not writable, go to the Traffic Forecast settings page for instructions on how to manually create this optimized endpoint file.
...
- Use value from `Traffic_Forecast_USE_CUSTOM_ENDPOINT` if it is defined.
...
- Fix code for custom referrer blocklist using filter `Traffic_Forecast_referrer_blocklist`.
...
- Add filter hook: `Traffic_Forecast_referrer_blocklist` ([example](https://github.com/Trailblazers/Traffic-Forecast/blob/master/code-snippets/add-domains-to-referrer-blocklist.php))
- Add filter hook: `Traffic_Forecast_ignore_referrer_url` ([example](https://github.com/Trailblazers/Traffic-Forecast/blob/master/code-snippets/ignore-some-referrer-urls.php))
...
- Don't attempt to install custom endpoint if it was manually installed (using the `Traffic_Forecast_USE_CUSTOM_ENDPOINT` constant).
- Revert to using `home_url()` for the tracker endpoint URL.


#### 1.0.12 - May 14, 2020

- Add filter `Traffic_Forecast_honor_dnt` to allow ignoring DoNotTrack.
...
- Add setting for specifying default date period when opening Forecast dashboard.
- Add Chrome-Lighthouse to list of ignored HTTP user agents.
- Show notice on Forecast dashboard page when buffer file is not writable.
...
- Expand referrer aggregation logic. If you have any URL's you would like to see combined into a single domain, please [post them here](https://github.com/Trailblazers/Traffic-Forecast/issues/43).
...
- Remember view period when navigating away from Forecast dashboard.
- Add filter hook to prevent loading the tracking script: `Traffic_Forecast_load_tracking_script`
...
- Add capabilities `view_Traffic_Forecast` and `manage_Traffic_Forecast` to control whether a user role can view or manage statistics.
...
- Show total size of Traffic Forecast' database tables on settings page.
...
- Add `[Traffic_Forecast_most_viewed_posts]` shortcode.
