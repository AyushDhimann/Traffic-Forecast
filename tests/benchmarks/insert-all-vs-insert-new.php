...
$wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}Traffic_Forecast_bench_referrer_urls");
$wpdb->query("CREATE TABLE {$wpdb->prefix}Traffic_Forecast_bench_referrer_urls (
...
$seed_sql = $wpdb->prepare("INSERT INTO {$wpdb->prefix}Traffic_Forecast_bench_referrer_urls(url) VALUES {$placeholders}", $values);
$wpdb->query($seed_sql);
...
$time = bench(function() use($wpdb, $all_urls) {
	...
	$placeholders = join(',', $placeholders);
	$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}Traffic_Forecast_bench_referrer_urls(url) VALUES {$placeholders} ON DUPLICATE KEY UPDATE url = url", $all_urls));
}, 1);
$total_time += $time;
...
$time = bench(function() use($wpdb, $all_urls) {
	$placeholders = array_fill(0, count($all_urls), '%s');
	$placeholders = join(',', $placeholders);
	$urls = $wpdb->get_results($wpdb->prepare("SELECT id, url FROM {$wpdb->prefix}Traffic_Forecast_bench_referrer_urls WHERE url IN({$placeholders})", $all_urls));
}, 3);
$total_time += $time;
...
echo PHP_EOL;
$wpdb->query("TRUNCATE {$wpdb->prefix}Traffic_Forecast_bench_referrer_urls");
$wpdb->query($seed_sql);
...
$time = bench(function() use($wpdb, $all_urls) {
	$placeholders = array_fill(0, count($all_urls), '%s');
	$placeholders = join(',', $placeholders);
	$results = $wpdb->get_results($wpdb->prepare("SELECT id, url FROM {$wpdb->prefix}Traffic_Forecast_bench_referrer_urls WHERE url IN({$placeholders})", $all_urls));
...
$time = bench(function() use($wpdb, $new_urls) {
	...
	$placeholders = join(',', $placeholders);
	$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}Traffic_Forecast_bench_referrer_urls(url) VALUES {$placeholders}", $values));
}, 1);
$total_time += $time;
